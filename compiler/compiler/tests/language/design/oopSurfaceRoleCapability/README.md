...
Status: design-only proof package after the language-test split.
...
Buildable proof input lives under:

```text
qtlang/compiler/tests/language/positive/oopSurfaceRoleCapability
```

Expected-fail fixtures live under:

```text
qtlang/compiler/tests/language/negative/oopSurfaceRoleCapability
```
...
Input kinds are separated before any broad qtlc1 language-test command runs:
...
The positive directory already contains a minimal buildable package. As qtlc0
learns the full centralized surface model, promote design cases into focused
positive source files behind that package:

```text
positive/oopSurfaceRoleCapability/
  quantum.toml
  module.toml
  mod.qn
  main.qn
...
Compile-fail commands after the expected-failure harness supports single-file
negative cases:
